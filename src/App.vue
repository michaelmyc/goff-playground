...
watch(proxyUrl, async (newUrl) => {
  try {
    ...
    error.value = err instanceof Error ? err.message : 'Failed to initialize provider'
...
const evaluateFlag = async (flagData: any) => {
  ...
  try {
    ...
    switch (flagData.flagType) {
      case 'boolean':
        details = client.getBooleanDetails(flagData.flagName, flagData.defaultValue)
        result = {
          value: details.value,
        ...
        break
      case 'string':
        details = client.getStringDetails(flagData.flagName, flagData.defaultValue)
        result = {
          value: details.value,
          reason: details.reason,
          variant: details.variant,
        ...
        break
      case 'number':
        details = client.getNumberDetails(flagData.flagName, flagData.defaultValue)
        result = {
          value: details.value,
          reason: details.reason,
          variant: details.variant,
        ...
        break
      case 'object':
        details = client.getObjectDetails(flagData.flagName, flagData.defaultValue)
        result = {
          value: details.value,
          reason: details.reason,
          variant: details.variant,
