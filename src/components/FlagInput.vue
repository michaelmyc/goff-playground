...
import { ref, onMounted, watch, onMounted as onMountedVue } from 'vue'
...
watch(() => formData.value.flagType, (newType) => {
  switch (newType) {
    case 'boolean':
...
const validateDefaultValue = () => {
  ...
  switch (formData.value.flagType) {
    case 'boolean':
      if (formData.value.defaultValue !== 'true' && formData.value.defaultValue !== 'false') {
        defaultValueError.value = 'Boolean default value must be "true" or "false"'
        return false
      }
      break
    case 'number':
...
const handleSubmit = () => {
  ...
  let contextObj = validateAndParseContext()
...
<template>
  <div class="flag-input">
    ...
    <div class="form-group">
      ...
      >
      <button type="button" @click="formData.targetingKey = uuidv4()" class="generate-btn">
        Generate UUID
      </button>
    </div>
